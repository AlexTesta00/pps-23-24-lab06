...
enum List[A]:
  ...
  def span(predicate: A => Boolean): (List[A], List[A]) =
    foldLeft((List.Nil(), List.Nil()))((i, e) =>
      (e, i) match
        case (e, (l1, l2)) if predicate(e) && l2.isEmpty() =>
          (l1.append(List.apply(e)), l2)
        case (e, (l1, l2)) => (l1, l2.append(List.apply(e)))
    )

  def takeRight(n: Int): List[A] = this.zipWithIndex.filter(_._2 >= this.length() - n).map(_._1)

  def collect(predicate: PartialFunction[A, A]): List[A] = filter(e => predicate.isDefinedAt(e)).map(e => predicate(e))
